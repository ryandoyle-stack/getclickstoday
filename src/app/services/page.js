    ...
    <>
      ...
      <div className="grid-bg" style={{position:'relative',overflow:'hidden'}}>
        ...
        <section style={{maxWidth:'1280px',margin:'0 auto',padding:'80px 52px 60px',position:'relative',zIndex:2}}>
          ...
          <h1 style={{fontFamily:'var(--font-display)',fontWeight:800,fontSize:'clamp(44px,6.5vw,84px)',lineHeight:0.98,letterSpacing:'-0.04em',marginBottom:'24px',maxWidth:'900px'}}>
            Everything you need<br />to <em style={{fontStyle:'normal',color:'var(--pink)'}}>grow.</em>
          </h1>
          <p style={{fontSize:'17px',fontWeight:300,color:'var(--muted)',maxWidth:'640px',lineHeight:1.75,marginBottom:'40px'}}>
      ...
      <section style={{background:'var(--bg)'}} className="dot-bg">
        <div className="wrap" style={{padding:'80px 52px'}}>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(320px,1fr))',gap:'1px',background:'var(--border)',border:'1px solid var(--border)',borderRadius:'8px',overflow:'hidden'}} className="fade-up">
            {services.map((s,i) => (
...
export const metadata = {
  title: 'Marketing Services — GetClicksToday | South Florida',
  description: 'Full-service marketing for South Florida businesses — social media, SEO, paid ads, email, content, GBP, and reputation management. Starting at $500/mo.',
}
